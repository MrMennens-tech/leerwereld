...
import { Themes } from './themes.js?v=3';
import { AudioEngine, createSoundController } from './audio-engines.js?v=3';
...
export const Utils = {
    ...
    getThemeItems: function () {
        ...
        const theme = allThemes[Settings.state.theme] || allThemes['sea'];
        theme.scenes = {
            drive: theme.items[0],
            pop: theme.items[1],
    ...
    renderItemVisual: function (item) {
        if (item.type === 'image') {
            return `<img src="${item.i}" style="width:1em; height:1em; object-fit:contain; vertical-align:middle;">`;
        }
        return item.i;
    },

    getSoundInfo: function (item) {
        if (item.audioType === 'mixkit') return { id: item.s, type: 'mixkit' };
        if (item.audioType === 'url') return { id: item.s, type: 'url' };
        return item.s;
    }
};
...
export const ThemeSelector = {
    ...
    init() {
        // Populeer themes pas wanneer nodig (lazy loading)
        this.updateMainMenuThemeIcon(); // Toon het actieve thema-icoon bij opstarten

        // Close button
        const closeBtn = document.getElementById('close-theme-selector-button');
        if (closeBtn) {
            closeBtn.addEventListener('click', () => {
                Navigation.showView('main-menu-view');
            });
        }
        ...
        if (themeGrid) {
            themeGrid.addEventListener('click', e => {
                ...
                this.updateMainMenuThemeIcon(); // Update het icoon in het hoofdmenu
                TTS.speak(Utils.themes[themeId].displayName);
                SoundController.play('klik');

                // Ga automatisch terug naar het hoofdmenu
                setTimeout(() => {
                    Navigation.showView('main-menu-view');
                }, 300); // Korte delay zodat de gebruiker de selectie ziet
            });
        }
    },

    populateThemes() {
        // Voeg themes alleen toe als ze nog niet toegevoegd zijn
        if (this.themesPopulated) return;

        const grid = document.getElementById('theme-grid');
        if (!grid) return;

        const allThemes = ConfigManager.getAllThemes();
        grid.innerHTML = Object.keys(allThemes).map(themeId => {
            const theme = allThemes[themeId];
            return `<div class="theme-card scannable" data-theme="${themeId}">
    ...
    updateMainMenuThemeIcon() {
        // Update het icoon van de Thema's kaart in het hoofdmenu
        const themeCard = document.getElementById('open-theme-selector');
        if (!themeCard) {
            // Element bestaat nog niet, probeer het later opnieuw
            setTimeout(() => this.updateMainMenuThemeIcon(), 100);
            return;
        }

        const iconElement = themeCard.querySelector('.icon');
        if (iconElement) {
            const currentTheme = ConfigManager.getAllThemes()[Settings.state.theme];
            if (currentTheme) {
                iconElement.textContent = currentTheme.icon;
                console.log(`Theme icon updated to: ${currentTheme.icon} (${currentTheme.displayName})`);
            }
        }
    }
};
...
export const Navigation = {
    init() {
        ...
        document.querySelector('.game-grid').addEventListener('click', e => {
            ...
            if (card.id === 'open-theme-selector') {
                ThemeSelector.populateThemes(); // Laad themes als nog niet geladen
        ...
        // Advanced config button in settings
        const openAdvBtn = document.getElementById('open-advanced-config');
        if (openAdvBtn) {
            openAdvBtn.addEventListener('click', () => {
                ConfigEditor.init();
                this.showView('advanced-config-view');
            });
        }

        // Close advanced config
        const closeAdvBtn = document.getElementById('close-advanced-config');
        if (closeAdvBtn) {
...
export const Games = {
    'interactive-scene': {
        ...
        setupLevel() {
            ...
            itemsToDisplay.forEach((item, index) => {
                ...
                el.innerHTML = Utils.renderItemVisual(item);
                el.dataset.name = item.n;
                el.dataset.soundInfo = JSON.stringify(Utils.getSoundInfo(item));
        ...
        handleClick(e) {
            ...
            SoundController.play(JSON.parse(target.dataset.soundInfo));
            if (!this.useWords) {
    ...
    'moving-targets': {
        ...
        addTarget() {
            ...
            el.innerHTML = Utils.renderItemVisual(item);
            const soundInfo = Utils.getSoundInfo(item);
            ...
            el.addEventListener('click', () => {
                SoundController.play(soundInfo);
                TTS.speak(item.n);
    ...
    'same-or-different': {
        ...
        setupLevel() {
            ...
            const referenceContent = this.useWords
                ...
                : `<div class="sod-object ${referenceTransform} ${referenceSize}">${Utils.renderItemVisual(this.correctAnswer)}</div>`;
            ...
            document.getElementById('sod-choice-area').innerHTML = Utils.shuffleArray(choices).map(c => {
                ...
                return `<div class="sod-choice-card scannable" data-name="${c.n}" data-sound-info='${JSON.stringify(Utils.getSoundInfo(c))}' data-icon="${c.i}" data-correct="${c.isCorrect}">
                    <div class="${classes}">${Utils.renderItemVisual(c)}</div>
                </div>`;
            }).join('');
        ...
        checkAnswer(card) {
            SoundController.play(JSON.parse(card.dataset.soundInfo));
    ...
    'counting': {
        ...
        setupLevel() {
            ...
            for (let i = 0; i < this.config.max; i++) {
                sourceArea.innerHTML += `<div class="counting-object scannable" data-sound-info='${JSON.stringify(Utils.getSoundInfo(item))}'>${Utils.renderItemVisual(item)}</div>`;
            }
    ...
    'math': {
        ...
        setupLevel() {
            ...
            if (this.useIcons && n1 <= 20 && n2 <= 20) {
                // Toon met icoontjes
                const item = Utils.getThemeItems().items[0];
                const createGroup = n => `<div class="math-object-group">${Array(n).fill(`<div class="math-object">${Utils.renderItemVisual(item)}</div>`).join('')}</div>`;
        ...
        setupLevel() {
            ...
            grid.innerHTML = cards.map(item => {
                const content = item.showWord
                    ...
                    : Utils.renderItemVisual(item);

                return `<div class="memory-card scannable" data-item="${item.i}" data-name="${item.n}" data-sound-info='${JSON.stringify(Utils.getSoundInfo(item))}'>
...
// --- EMOJI SUGGESTIE DATA ---
const EmojiSuggestions = {
    // Zee
    'vis': '🐠', 'haai': '🦈', 'krab': '🦀', 'octopus': '🐙', 'walvis': '🐳', 'dolfijn': '🐬', 'zeester': '⭐', 'schelp': '🐚',
    // Dieren
    'hond': '🐶', 'kat': '🐱', 'muis': '🐭', 'konijn': '🐰', 'vos': '🦊', 'beer': '🐻', 'panda': '🐼', 'koala': '🐨', 'tijger': '🐯', 'leeuw': '🦁', 'koe': '🐮', 'varken': '🐷', 'kikker': '🐸', 'aap': '🐵', 'kip': '🐔', 'pinguïn': '🐧', 'vogel': '🐦', 'eend': '🦆', 'uil': '🦉', 'slang': '🐍', 'schildpad': '🐢',
    // Fruit & Eten
    'appel': '🍎', 'peer': '🍐', 'sinaasappel': '🍊', 'citroen': '🍋', 'banaan': '🍌', 'watermeloen': '🍉', 'druiven': '🍇', 'aardbei': '🍓', 'kers': '🍒', 'perzik': '🍑', 'ananas': '🍍', 'kokosnoot': '🥥', 'pizza': '🍕', 'ijs': '🍦', 'taart': '🍰', 'snoep': '🍬',
    // Voertuigen
    'auto': '🚗', 'bus': '🚌', 'trein': '🚂', 'vliegtuig': '✈️', 'raket': '🚀', 'fiets': '🚲', 'boot': '🚢', 'tractor': '🚜',
    // Voorwerpen
    'bal': '⚽', 'cadeau': '🎁', 'bel': '🔔', 'boek': '📖', 'zon': '☀️', 'maan': '🌙', 'ster': '⭐', 'wolk': '☁️', 'regen': '🌧️', 'sneeuw': '❄️', 'hart': '❤️'
};

// --- MIXKIT SUGGESTIE DATA ---
// AudioSuggestions - Geverifieerde Mixkit ID's
// Gebruik mixkit-browser.html om meer ID's te vinden en te verifiëren!
const AudioSuggestions = {
    // Alleen geverifieerde geluiden
    'tada': '2639', 'fanfare': '2639', 'succes': '2571', 'fout': '2568',
    'klik': '2867', 'bel': '1111', 'notificatie': '2869', 'alert': '2870',
    'sparkle': '2003', 'magie': '2003', 'level': '2016', 'munt': '2019',
    'vogel': '2517', 'retro': '2574', 'jump': '2575'
};

// --- SOUND PICKER DATABASE ---
// CommonSounds - Deze worden NIET meer gebruikt door de app.
// Gebruik mixkit-browser.html om correcte Mixkit ID's te vinden!
const CommonSounds = {
    'Notificaties': [
        { n: 'Notificatie Pop', id: '2869' }, { n: 'Alert Ping', id: '2870' }, 
        { n: 'Succes Chime', id: '2571' }, { n: 'Fout Buzz', id: '2568' },
        { n: 'Klik', id: '2867' }, { n: 'Bel Ding', id: '1111' }
    ],
    'Muzikaal': [
        { n: 'Tada Fanfare', id: '2639' }, { n: 'Magische Sparkle', id: '2003' },
        { n: 'Level Up', id: '2016' }, { n: 'Coin Collect', id: '2019' }
    ],
    'Retro': [
        { n: 'Retro Blip', id: '2574' }, { n: '8-bit Jump', id: '2575' }
    ],
    'Natuur': [
        { n: 'Vogel Tjilp', id: '2517' }
    ]
};

// --- EMOJI PICKER (v2 Uitbreiding) ---
export const EmojiPicker = {
    ...
    async load() {
        ...
        const resultsArea = document.getElementById('emoji-search-results');
        resultsArea.innerHTML = '<p style="text-align center; padding: 2vmin; width: 100%">⏳ Lijst laden...</p>';
        try {
            const resp = await fetch('https://unpkg.com/emojibase-data@latest/nl/compact.json');
            this.data = await resp.json();
            this.isLoaded = true;
            this.search(''); // Toon basis lijst
        } catch (e) {
            console.error('Emoji load failed', e);
            resultsArea.innerHTML = '<p style="color: red; padding: 2vmin;">Lijst laden mislukt. Check je internet.</p>';
        }
    },
    ...
    search(query) {
        ...
        let filtered = [];
        if (!queryLower) {
            // Toon populaire groepen als er geen query is
            filtered = this.data.slice(0, 100);
        } else {
            filtered = this.data.filter(emoji => {
                const labelMatch = emoji.label.toLowerCase().includes(queryLower);
                const tagMatch = emoji.tags && emoji.tags.some(tag => tag.toLowerCase().includes(queryLower));
                return labelMatch || tagMatch;
            }).slice(0, 80); // Limiteer resultaten voor performance
        }

        resultsArea.innerHTML = filtered.map(emoji => `
            <div class="config-item-card scannable" style="height: 60px; padding: 0.5vmin;" data-emoji="${emoji.unicode}" title="${emoji.label}">
                <div class="item-visual" style="font-size: 2.2rem;">${emoji.unicode}</div>
            </div>
        `).join('');

        // Resultaten click handlers
        resultsArea.querySelectorAll('.config-item-card').forEach(el => {
            el.addEventListener('click', () => {
                const selectedEmoji = el.dataset.emoji;
                document.getElementById('item-edit-emoji').value = selectedEmoji;
                document.getElementById('item-image-preview').textContent = selectedEmoji;
                this.close();
            });
        });
    },
    ...
    open() {
        document.getElementById('emoji-picker-modal').classList.add('show');
        this.load();
        document.getElementById('emoji-search-input').focus();
    },

    close() {
        document.getElementById('emoji-picker-modal').classList.remove('show');
    },
    ...
    init() {
        ...
        document.getElementById('open-emoji-picker').onclick = () => this.open();
        document.getElementById('close-emoji-picker').onclick = () => this.close();
    }
};

// --- AUDIO RECORDER (v2 Uitbreiding) ---
    ...
    async start() {
        try {
            ...
            this.mediaRecorder.onstop = async () => {
                ...
                const base64 = await ConfigManager.fileToBase64(audioBlob);
                if (this.onRecordComplete) this.onRecordComplete(base64);
                // Stop alle tracks om de mic uit te zetten
...
// --- SOUND PICKER (v2 Uitbreiding) ---
export const SoundPicker = {
    activeCategory: 'Dieren',

    render() {
        const categoriesArea = document.getElementById('sound-categories');
        const resultsArea = document.getElementById('sound-search-results');

        // Render Categories
        categoriesArea.innerHTML = Object.keys(CommonSounds).map(cat => `
            <button class="config-action-btn scannable ${cat === this.activeCategory ? 'active' : 'secondary'}" 
                    style="padding: 1vmin 2vmin; white-space: nowrap;">${cat}</button>
        `).join('');

        categoriesArea.querySelectorAll('button').forEach(btn => {
            btn.onclick = () => {
                this.activeCategory = btn.textContent;
                this.render();
            };
        });

        // Render Results
        const sounds = CommonSounds[this.activeCategory];
        resultsArea.innerHTML = sounds.map(s => `
            <div class="config-item-card scannable" style="height: 100px; flex-direction: column;" data-id="${s.id}">
                <div class="item-visual" style="font-size: 2rem;">🔊</div>
                <div class="item-name" style="font-size: 0.8rem;">${s.n}</div>
            </div>
        `).join('');

        resultsArea.querySelectorAll('.config-item-card').forEach(el => {
            el.addEventListener('click', () => {
                const soundId = el.dataset.id;
                document.getElementById('item-edit-sound-mixkit').value = soundId;
                SoundController.play({ id: soundId, type: 'mixkit' });
                this.close();
            });
        });
    },
    ...
    search(query) {
        ...
        let allSounds = [];
        Object.values(CommonSounds).forEach(catSounds => allSounds.push(...catSounds));

        const filtered = allSounds.filter(s => s.n.toLowerCase().includes(queryLower));
    ...
    open() {
        document.getElementById('sound-picker-modal').classList.add('show');
        this.render();
    },

    close() {
        document.getElementById('sound-picker-modal').classList.remove('show');
    },

    init() {
        document.getElementById('sound-search-input').oninput = (e) => this.search(e.target.value);
        // open-sound-picker is nu een link naar mixkit-browser.html
        document.getElementById('close-sound-picker').onclick = () => this.close();
    }
};

// --- CONFIG EDITOR (v2 Kern) ---
export const ConfigEditor = {
    ...
    renderThemeList() {
        ...
        list.innerHTML = Object.entries(themes).map(([id, theme]) => `
            <div class="config-list-item ${this.activeThemeId === id ? 'active' : ''}" data-id="${id}">
                <div class="item-meta">
                    <span>${theme.icon || '📁'}</span>
                    <span>${theme.displayName}</span>
                </div>
                ${!Themes[id] ? `<span class="delete-small" data-id="${id}">🗑️</span>` : ''}
            </div>
        `).join('');

        list.querySelectorAll('.config-list-item').forEach(el => {
            el.addEventListener('click', (e) => {
                if (e.target.classList.contains('delete-small')) return;
                this.selectTheme(el.dataset.id);
            });
        });

        list.querySelectorAll('.delete-small').forEach(el => {
            el.addEventListener('click', (e) => {
                if (confirm('Thema verwijderen?')) {
                    ConfigManager.deleteTheme(el.dataset.id);
                    this.activeThemeId = null;
                    this.renderThemeList();
                    document.getElementById('theme-editor-content').style.display = 'none';
                    document.getElementById('editor-theme-title').textContent = 'Selecteer een thema';
                }
            });
        });
    },
    ...
    selectTheme(id) {
        ...
        const themes = ConfigManager.getAllThemes();
        this.currentThemeData = JSON.parse(JSON.stringify(themes[id])); // Deep clone
    ...
    showThemeEditor() {
        document.getElementById('theme-editor-content').style.display = 'block';
        document.getElementById('export-theme-btn').style.display = 'block';
        document.getElementById('save-theme-btn').style.display = 'block';
        document.getElementById('editor-theme-title').textContent = `Thema: ${this.currentThemeData.displayName}`;

        document.getElementById('edit-theme-name').value = this.currentThemeData.displayName;
        document.getElementById('edit-theme-icon').value = this.currentThemeData.icon || '';
        document.getElementById('edit-theme-bg').value = this.currentThemeData.background || '';

        this.renderItemList();
    },

    renderItemList() {
        const list = document.getElementById('config-item-list');
        list.innerHTML = this.currentThemeData.items.map((item, index) => {
            let visual = item.i;
            if (item.type === 'image') {
                visual = `<img src="${item.i}" alt="${item.n}">`;
            }
            return `
                <div class="config-item-card scannable" data-index="${index}">
                    <div class="delete-overlay" data-index="${index}">×</div>
        ...
        list.querySelectorAll('.config-item-card').forEach(el => {
            el.addEventListener('click', (e) => {
                if (e.target.classList.contains('delete-overlay')) {
                    this.currentThemeData.items.splice(parseInt(e.target.dataset.index), 1);
                    this.renderItemList();
                    return;
                }
                this.openItemEditor(parseInt(el.dataset.index));
            });
        });
    },
    ...
    openItemEditor(index) {
        ...
        const item = index === -1
            ? { i: '❓', n: 'Nieuw Item', s: 'klik', type: 'emoji', audioType: 'builtin' }
            : this.currentThemeData.items[index];

        document.getElementById('item-edit-name').value = item.n;
        document.getElementById('item-edit-emoji').value = item.type === 'emoji' ? item.i : '';
        document.getElementById('item-edit-sound-mixkit').value = item.audioType === 'mixkit' ? item.s : '';
        document.getElementById('item-edit-sound-builtin').value = item.audioType === 'builtin' ? (typeof item.s === 'string' ? item.s : 'klik') : 'klik';

        // Toggles
        this.setItemAppearanceMode(item.type || 'emoji');

        let audioMode = item.audioType || 'builtin';
        if (audioMode === 'url' && item.s.startsWith('data:')) audioMode = 'upload';
        this.setAudioMode(audioMode);

        if (item.type === 'image') {
            document.getElementById('item-image-preview').innerHTML = `<img src="${item.i}">`;
        } else {
            document.getElementById('item-image-preview').textContent = item.i || '❓';
        }

        document.getElementById('item-editor-modal').classList.add('show');
    },

    setItemAppearanceMode(mode) {
        const isEmoji = mode === 'emoji';
        document.getElementById('toggle-item-emoji').classList.toggle('active', isEmoji);
        document.getElementById('toggle-item-image').classList.toggle('active', !isEmoji);
        document.getElementById('emoji-input-area').style.display = isEmoji ? 'block' : 'none';
        document.getElementById('image-input-area').style.display = !isEmoji ? 'block' : 'none';
    },

    setAudioMode(mode) {
        const isBuiltin = mode === 'builtin';
        const isMixkit = mode === 'mixkit';
        const isUpload = mode === 'upload';

        document.getElementById('toggle-sound-builtin').classList.toggle('active', isBuiltin);
        document.getElementById('toggle-sound-mixkit').classList.toggle('active', isMixkit);
        document.getElementById('toggle-sound-upload').classList.toggle('active', isUpload);

        document.getElementById('builtin-sound-area').style.display = isBuiltin ? 'block' : 'none';
        document.getElementById('mixkit-sound-area').style.display = isMixkit ? 'block' : 'none';
        document.getElementById('upload-sound-area').style.display = isUpload ? 'block' : 'none';
    },

    populateBuiltinSounds() {
        const select = document.getElementById('item-edit-sound-builtin');
        if (select.options.length > 0) return;

        const sounds = Object.keys(SoundController.soundMaps.webaudio);
        select.innerHTML = sounds.map(s => `<option value="${s}">${s}</option>`).join('');
    },

    setupEventListeners() {
        // Theme level edits
        document.getElementById('edit-theme-name').oninput = (e) => {
            this.currentThemeData.displayName = e.target.value;
            document.getElementById('editor-theme-title').textContent = `Thema: ${e.target.value}`;
        };
        document.getElementById('edit-theme-icon').oninput = (e) => this.currentThemeData.icon = e.target.value;
        document.getElementById('edit-theme-bg').oninput = (e) => this.currentThemeData.background = e.target.value;

        // Save Theme
        document.getElementById('save-theme-btn').onclick = () => {
            ConfigManager.saveTheme(this.activeThemeId, this.currentThemeData);
            this.renderThemeList();
        ...
        document.getElementById('export-theme-btn').onclick = () => {
            const code = `'${this.activeThemeId}': ${JSON.stringify(this.currentThemeData, null, 4)},`;
        ...
        document.getElementById('copy-export-btn').onclick = () => {
            ...
            TTS.speak('Code gekopieerd naar klembord.');
        };

        // Download as JSON
        document.getElementById('download-theme-btn').onclick = () => {
            const dataStr = "data:text/json;charset=utf-8," + encodeURIComponent(JSON.stringify(this.currentThemeData, null, 4));
            const downloadAnchorNode = document.createElement('a');
            downloadAnchorNode.setAttribute("href", dataStr);
            downloadAnchorNode.setAttribute("download", `leerzone-thema-${this.activeThemeId}.json`);
            document.body.appendChild(downloadAnchorNode);
            downloadAnchorNode.click();
            downloadAnchorNode.remove();
            TTS.speak('Thema bestand gedownload.');
        };

        // Email Theme
        document.getElementById('email-theme-btn').onclick = () => {
            const subject = encodeURIComponent(`Nieuw LeerZone Thema: ${this.currentThemeData.displayName}`);
            const body = encodeURIComponent(
                "Hallo,\n\nHierbij een nieuw thema voor de LeerZone app.\n\n" +
                "Kopieer de onderstaande code en plak deze in themes.js:\n\n" +
                `'${this.activeThemeId}': ` + JSON.stringify(this.currentThemeData, null, 4)
            );

            // Check lengte (Base64 vreet veel tekens, mailto heeft limiet van ~2000 chars)
            if (body.length > 2000) {
                alert("Dit thema bevat grote afbeeldingen en is te groot voor de e-mail tekst. Gebruik de 'Download .JSON' knop en voeg het bestand handmatig toe als bijlage aan je mail.");
                return;
            }
        ...
        // Add Theme
        document.getElementById('add-new-theme').onclick = () => {
            const id = 'custom_' + Date.now();
            const newTheme = {
                displayName: 'Nieuw Thema',
                icon: '📁',
                background: 'linear-gradient(to bottom, #ffffff, #eeeeee)',
                items: [{ i: '⭐', n: 'Ster', s: 'glinstering', type: 'emoji', audioType: 'builtin' }]
            };
            ConfigManager.saveTheme(id, newTheme, true);
            this.selectTheme(id);
        };

        // Reset All
        document.getElementById('reset-all-config').onclick = () => {
            if (confirm('Weet je zeker dat je ALLE aanpassingen wilt verwijderen?')) {
        ...
        // Item Modal Toggles
        document.getElementById('item-edit-name').oninput = (e) => {
            const val = e.target.value.toLowerCase().trim();
            // Emoji Suggestie
            if (EmojiSuggestions[val]) {
                const emojiInput = document.getElementById('item-edit-emoji');
                if (!emojiInput.value || emojiInput.value === '❓' || emojiInput.value === '⭐') {
                    emojiInput.value = EmojiSuggestions[val];
                    if (document.getElementById('toggle-item-emoji').classList.contains('active')) {
                        document.getElementById('item-image-preview').textContent = EmojiSuggestions[val];
                    }
                }
            }
            // Audio Suggestie (Mixkit)
            if (AudioSuggestions[val]) {
                const mixkitInput = document.getElementById('item-edit-sound-mixkit');
                if (!mixkitInput.value) {
                    mixkitInput.value = AudioSuggestions[val];
                }
            }
        };

        document.getElementById('toggle-item-emoji').onclick = () => this.setItemAppearanceMode('emoji');
        document.getElementById('toggle-item-image').onclick = () => this.setItemAppearanceMode('image');
        document.getElementById('toggle-sound-builtin').onclick = () => this.setAudioMode('builtin');
        document.getElementById('toggle-sound-mixkit').onclick = () => this.setAudioMode('mixkit');
        document.getElementById('toggle-sound-upload').onclick = () => this.setAudioMode('upload');

        document.getElementById('item-edit-emoji').oninput = (e) => {
            document.getElementById('item-image-preview').textContent = e.target.value;
        };

        // File uploads (Image & Audio)
        document.getElementById('item-edit-file').onchange = async (e) => {
            if (e.target.files && e.target.files[0]) {
                const base64 = await ConfigManager.fileToBase64(e.target.files[0]);
                document.getElementById('item-image-preview').innerHTML = `<img src="${base64}">`;
                this.tempImageData = base64;
            }
        };

        document.getElementById('item-edit-sound-file').onchange = async (e) => {
            if (e.target.files && e.target.files[0]) {
                const base64 = await ConfigManager.fileToBase64(e.target.files[0]);
                this.tempAudioData = base64;
                TTS.speak('Geluid geselecteerd. Druk op Test om te horen.');
            }
        };

        // UI Feedback & Action for Recording
        const recordBtn = document.getElementById('start-record-btn');
        recordBtn.onmousedown = () => {
            AudioRecorder.onRecordComplete = (base64) => {
                this.tempAudioData = base64;
                TTS.speak('Opname voltooid.');
        ...
        recordBtn.onmouseup = () => {
            AudioRecorder.stop();
            recordBtn.textContent = '🎙️ Neem op';
        };
        // Touch support
        ...
        document.getElementById('test-sound-btn').onclick = () => {
            const isMixkit = document.getElementById('toggle-sound-mixkit').classList.contains('active');
            const isUpload = document.getElementById('toggle-sound-upload').classList.contains('active');

            if (isUpload) {
                if (this.tempAudioData) {
                    SoundController.play({ id: this.tempAudioData, type: 'url' });
                } else {
                    const item = this.editingItemIndex !== -1 ? this.currentThemeData.items[this.editingItemIndex] : null;
                    if (item && item.audioType === 'url' && item.s.startsWith('data:')) {
                        SoundController.play({ id: item.s, type: 'url' });
                    }
                }
                return;
            }

            const soundId = isMixkit
                ? document.getElementById('item-edit-sound-mixkit').value
                : document.getElementById('item-edit-sound-builtin').value;

            SoundController.play({ id: soundId, type: isMixkit ? 'mixkit' : 'builtin' });
        };

        // Modal Actions
        document.getElementById('cancel-item-edit').onclick = () => {
            document.getElementById('item-editor-modal').classList.remove('show');
            this.tempImageData = null;
            this.tempAudioData = null;
        };

        document.getElementById('save-item-edit').onclick = () => {
            const name = document.getElementById('item-edit-name').value;
            const isEmoji = document.getElementById('toggle-item-emoji').classList.contains('active');
            const isMixkit = document.getElementById('toggle-sound-mixkit').classList.contains('active');
            const isUpload = document.getElementById('toggle-sound-upload').classList.contains('active');

            let soundValue = '';
            let audioType = 'builtin';

            if (isMixkit) {
                soundValue = document.getElementById('item-edit-sound-mixkit').value;
                audioType = 'mixkit';
            } else if (isUpload) {
                soundValue = this.tempAudioData || (this.editingItemIndex !== -1 ? this.currentThemeData.items[this.editingItemIndex].s : '');
                audioType = 'url';
            } else {
                soundValue = document.getElementById('item-edit-sound-builtin').value;
                audioType = 'builtin';
            }
            ...
            const itemData = {
                ...
                i: isEmoji ? document.getElementById('item-edit-emoji').value : (this.tempImageData || (this.editingItemIndex !== -1 ? this.currentThemeData.items[this.editingItemIndex].i : '❓')),
                s: soundValue,
                audioType: audioType
            };

            if (this.editingItemIndex === -1) {
                this.currentThemeData.items.push(itemData);
            } else {
                this.currentThemeData.items[this.editingItemIndex] = itemData;
            }
        ...
        document.getElementById('add-item-to-theme').onclick = () => this.openItemEditor(-1);
    }
};
